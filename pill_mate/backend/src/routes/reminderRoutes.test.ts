...
describe('POST /reminder/', () => {
    ...
    it(
        ...
        async () => {
            ...
            expect(response.body).toStrictEqual({
                message: 'Your not allowed to add a reminder for an other user.',
            });
    ...
    it('should create the reminder', async () => {
        const getMedications = jest.fn();
        getMedications.mockResolvedValue([{ id: 1 }]);
        const createReminder = jest.fn();
        createReminder.mockResolvedValue({
    ...
    it(
        ...
        async () => {
            ...
            const response = await request(app)
                ...
                .set('x-remote-user-display-name', 'John Doe');
            expect(response.body).toStrictEqual({
                message: 'Your not allowed to modify this reminder.',
            });
            expect(response.status).toBe(403);
            expect(User.findOne).toHaveBeenCalledTimes(1);
            expect(User.findOne).toHaveBeenCalledWith({
    ...
    it(
        ...
        async () => {
            ...
            const response = await request(app)
                ...
                .set('x-remote-user-display-name', 'John Doe');
            expect(response.body).toStrictEqual({
                message: 'Your not allowed to delete this reminder.',
            });
            expect(response.status).toBe(403);
            expect(User.findOne).toHaveBeenCalledTimes(1);
            expect(User.findOne).toHaveBeenCalledWith({
